...
if (($ok && $OJ_FRIENDLY_LEVEL>2) ||
  ...
  !(                                                                                   // 默认禁止比赛中查看WA对比和RE详情
    ...
    if (strpos($row['error'], "judge")!==false) {
        $view_reinfo =  "error1";
    } elseif (strpos($row['error'], "php")!==false) {
        $view_reinfo =  "error2";
    } elseif (strpos($row['error'], "PASS")!==false) {
        $view_reinfo =  "error3";
    } elseif ($OJ_SHOW_DIFF && $row && ($ok||$isRE) && ($OJ_TEST_RUN||is_valid($row['error'])||$ok)) {
