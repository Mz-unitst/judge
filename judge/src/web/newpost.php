...
<h2 style="margin:0px 10px">Post New Thread<?php if (isset($_REQUEST['cid']) && $_REQUEST['cid']!='') {
    echo ' For Contest '.intval($_REQUEST['cid']);
}?></h2>
<form action="post.php?action=new" method=post>
<input type=hidden name=cid value="<?php if (isset($_REQUEST['cid'])) {
    echo intval($_REQUEST['cid']);
}?>">
