...
<div style="text-align:left;font-size:80%;float:left;">[ <a href="newpost.php<?php if ($cid) {
    echo "?cid=$cid&pid=".$row['pid'];
} ?>">New Thread</a> ]</div>
<?php if ($isadmin) {
    ?><div style="font-size:80%; float:right"> Change sticky level to<?php $adminurl = "threadadmin.php?target=thread&tid={$tid}&action=";
    if ($row['top_level'] == 0) {
        echo "[ <a href=\"{$adminurl}sticky&level=3\">Level Top</a> ] [ <a href=\"{$adminurl}sticky&level=2\">Level Mid</a> ] [ <a href=\"{$adminurl}sticky&level=1\">Level Low</a> ]";
    } else {
        echo "[ <a href=\"{$adminurl}sticky&level=0\">Standard</a> ]";
    }
    ?> | <?php if ($row['status'] != 1) {
        echo(" [ <a  href=\"{$adminurl}lock\">Lock</a> ]");
    } else {
        echo(" [ <a href=\"{$adminurl}resume\">Resume</a> ]");
    }
...
<tr align=center class='ui top'>
	...
	<a href="discuss.php<?php if ($row['pid']!=0 && $row['cid']!=null) {
	...
	<?php if ($row['pid']!=0) {
	    ...
	    echo "MainBoard";
	}?></a> >> <?php echo nl2br(htmlentities($row['title'], ENT_QUOTES, "UTF-8"));?></td>
</tr>
	...
	<td>
     ...
     <div style="display:inline;text-align:left; float:left; margin:0 10px"><a href="../userinfo.php?user=<?php echo $row['author_id']?>"><?php echo $row['author_id']; ?> </a> @ <?php echo $row['time']; ?></div>
		<div class="mon" style="display:inline;text-align:right; float:right">
			...
			<span>[ <a href="
				<?php if ($row['status']==0) {
				    echo $url."disable\">Disable";
				} else {
				    echo $url."resume\">Resume";
				}
			    ?> </a> ]</span>
		...
		<div id="post<?php echo $row['rid'];?>" class=content style="text-align:left; clear:both; margin:10px 30px">
			<?php	if ($row['status'] == 0) {
			    ...
			    if (!$isuser || $isadmin) {
			        echo "<div style=\"border-left:10px solid gray\"><font color=red><i>Notice : <br>This reply is blocked by administrator.</i></font></div>";
			    }
			    if ($isuser || $isadmin) {
			        echo nl2br(htmlentities($row['content'], ENT_QUOTES, "UTF-8"));
			    }
