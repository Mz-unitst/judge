<?php
    ...
    if (isset($_SESSION[$OJ_NAME.'_'.'balloon'])) {
        ...
        foreach ($result as $row) {
            ...
            if ($first_blood[$row['pid']]==$row['user_id']) {
                $view_balloon[$i][2].=" First Blood!";
            }
            $view_balloon[$i][2].="</font>";
            $view_balloon[$i][3]="";
            if ($row['status']==1) {
                $view_balloon[$i][3].="<span class='btn btn-success'>$MSG_BALLOON_DONE</span>";
            } else {
                $view_balloon[$i][3].="<span class='btn btn-danger'>$MSG_BALLOON_PENDING</span>";
            }
